package org.s2.serialisation.xml;
 ...
 */
public class S2RmJaxbUtil extends JAXBUtil {
    ...
    private static synchronized void initJaxbContext() {
        if(jaxbContext == null) {
            try {
                ...
                classes.addAll(S2RmInfoLookup.getInstance().getRmTypeNameToClassMap().values());
    ...
    public static synchronized JAXBContext createRMContext() {
        try {
            List<Class<?>> classes = Lists.newArrayList(S2RmInfoLookup.getInstance().getRmTypeNameToClassMap().values());
            removeClasses(classes);
